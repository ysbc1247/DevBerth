...
- Treat a DevBerth display name, bundle identifier, frontmost window, and Accessibility application identity as insufficient proof of the running application. Release, Debug, development-control, UI-test, DerivedData, and temporary hosts may share `com.ysbc.devberth`; resolve the exact bundle path, executable path, PID, build/development mode, control socket, and persistence mode before using a host for validation, screenshots, performance evidence, or cleanup.
- Use the installed `/Applications/DevBerth.app` and its exact running executable as the default source for production/public visual or live-runtime evidence. Verify that it is not a development or fixture host, that it is bound to the production control path rather than `IPC/Development/control.sock`, and that its visible live state matches the intended production context. Never relabel a Debug/fixture capture as production evidence; discard and recapture whenever provenance is ambiguous.
- Stop or clean up a development host only after revalidating the exact task-owned PID, executable path, development flags, socket, and isolated store mode. Never quit, signal, or delete by product name or bundle identifier, and never target `/Applications/DevBerth.app`, the production control socket, the production SwiftData store, or unrelated user runtimes while cleaning development fixtures. Record source bundle/executable, capture time, mode, socket/store context, and every crop or redaction with retained visual evidence.
...
- Run trusted tools non-interactively with standard input closed and drain stdout and stderr continuously while they execute. Mark every parent-side command-capture and managed-log pipe descriptor close-on-exec before any subprocess can be spawned; a parallel long-running service must inherit only its intentional standard streams, never another operation's pipe endpoints. Runtime project inference must remain bounded and metadata-only; never open or parse an observed process's project files during a listener refresh.
...
- Keep passive Docker and listener refreshes inspection-only: they may read engine container metadata and published ports, but must not perform Compose control-scope verification or block runtime observation on project-file access. Passive Docker observation must use the batched container path without a separate availability command, refresh slower than listener state, back off exponentially while unavailable, and invalidate after manual refresh or a completed Docker mutation.
...
- Keep ongoing health work per service, generation-cancellable on stop/exit/deletion, suppressed during sleep, adaptive between recovery and stable cadence, jittered, and globally bounded to four concurrent batches. Saturated health sampling skips rather than queues delayed duplicate checks.
...
- Retain at most the newest 5,000 compatibility process-history records. Repair older stores at startup and prune with write headroom instead of fetching or deleting on every event.
...
- Collect resource usage through one bounded, batched `ps` reader per scheduled resource sample, with immediate sampling for PID-set changes and slower active/background/idle cadence than listener scans. Resource values are transient evidence; unavailable or malformed rows render as unavailable and never affect ownership or lifecycle authority.
- Route managed stdout/stderr chunks through `ServiceLogIngress`; batch before actor processing, disk append, and UI publication. Keep the in-memory entry count and persisted bytes bounded, preserve cross-chunk secret redaction, and make visible log views copy entries only when a lightweight revision changes.
- Keep one idempotent application-lifetime `PortMonitor` loop. Coalesce immediate refreshes behind any in-flight scan, suspend across system sleep, use visibility/stability-aware cadence, correlate Docker before semantic diffing, and never classify observation timestamps alone as runtime changes. Derive foreground surfaces from the actual `NSWindow` plus application activation, not SwiftUI appearance callbacks; treat a menu-bar popover as visible only while its backing window is key, and ignore duplicate surface callbacks. Publish the retained hidden snapshot exactly once when a monitoring surface becomes foreground-visible so activation never renders stale evidence. Observe high-numbered interface-bound UDP endpoint churn without extending transition cadence. Store resource evidence continuously but do not publish resource-only changes with no visible monitoring surface. Cache process metadata only behind a bounded native full-identity key that invalidates PID reuse, executable/argument identity, or current-directory changes.
