...
Every parent-side command-capture and managed-log pipe endpoint receives `FD_CLOEXEC` before DevBerth can spawn another process. The intended child receives only its duplicated standard-output and standard-error endpoints. This is required for dependency layers that launch siblings concurrently: without it, one long-running service can inherit a sibling fingerprint probe's writer and prevent that probe from ever observing EOF, leaving project startup permanently non-terminal even after the listener appears.
